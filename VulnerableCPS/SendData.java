	...
	try{
		while(getRunning()) {
			...
			if(data > 15 && data < 45) {
				...
				status = 0;
				SensorPanel.status.setText(source.getNode()+" detected normal eigen vector value "+data);
			}
			else if(data > 45 && data < 50) {
				source.setTotal(source.getTotal() + 1);
				source.setStrict(source.getStrict() + 1);
				status = 1;
				SensorPanel.status.setText(source.getNode()+" detected strict-stealthy eigen vector value "+data+" Normal sensor range value is 15 to 45");
			}
